...
signal dev_mode_requested
signal settings_requested
signal about_requested

@onready var _play_button: Button = %PlayButton
@onready var _dev_mode_button: Button = %DevModeButton
...
func _ready() -> void:
	...
	_dev_mode_button.pressed.connect(_on_dev_mode_pressed)
	_settings_button.pressed.connect(_on_settings_pressed)
	_about_button.pressed.connect(_on_about_pressed)
	_bind_hover_sounds()


func show_menu(focus_dev_mode: bool = false) -> void:
	visible = true
	if focus_dev_mode:
		_dev_mode_button.grab_focus()
	else:
